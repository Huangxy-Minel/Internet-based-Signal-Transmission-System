...
//UDP发送数据内容
static u8 udp_send_buf[802] = {0};
static u8 ad_msg[1600] = {0};
u8 *p = ad_msg;
...
void udp_demo_test(void)
{
	...
	udp_send_buf[800] = 0x11; //表明为从机2
	DP83640_selectPage(4);
	j = 0; //记录循环次数
	while(res==0)
	{
		if(udp_demo_flag&1<<6 && udp_demo_recvbuf[8] == 0x11)//是否收到数据?
		{
...
void UDP_SEND_TASK(struct udp_pcb *upcb)
{
	u8* p_temp = ad_msg;
	struct pbuf *ptr;
	ptr=pbuf_alloc(PBUF_TRANSPORT,802,PBUF_POOL); //申请内存
	memcpy(udp_send_buf, p_temp, 800);
	udp_send_buf[801] = 0x02;
	pbuf_take(ptr,(char*)udp_send_buf,802); //将ptpsync中的数据打包进pbuf结构中
	udp_send(upcb,ptr);	//udp发送数据 
	delay_ms(100);
	p_temp += 800;
	memcpy(udp_send_buf, p_temp, 800);
	udp_send_buf[801] = 0x00;
	pbuf_take(ptr,(char*)udp_send_buf,802); //将ptpsync中的数据打包进pbuf结构中
	udp_send(upcb,ptr);	//udp发送数据 
