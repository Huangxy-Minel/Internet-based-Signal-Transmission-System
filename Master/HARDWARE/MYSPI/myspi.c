...
void SPI_Init(void)
{
  ...
  GPIO_Initure.Pin=GPIO_PIN_4|GPIO_PIN_3|GPIO_PIN_1|GPIO_PIN_2; //PF6,1--SYNC,SCLK
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
  GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
  GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
  HAL_GPIO_Init(GPIOF,&GPIO_Initure);
	HAL_GPIO_WritePin(GPIOF,GPIO_PIN_4,GPIO_PIN_RESET);	//SYNC置0
	
	HAL_GPIO_WritePin(GPIOF,GPIO_PIN_3,GPIO_PIN_SET);	//SYNC置0
	
	HAL_GPIO_WritePin(GPIOF,GPIO_PIN_1,GPIO_PIN_RESET);	//SYNC置0
	
	HAL_GPIO_WritePin(GPIOF,GPIO_PIN_2,GPIO_PIN_RESET);	//SYNC置0
